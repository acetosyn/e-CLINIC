  ...
  recordsTriggers.forEach(btn => {
    btn.addEventListener("click", async (e) => {
      ...
      try {
        ...
        /* ------------------------------------------------------
           3️⃣ DYNAMICALLY LOAD records.js AFTER HTML IS INJECTED
        ------------------------------------------------------ */
        // Reset the guard flag so records.js can reinitialize
        window.__EPIC_RECORDS_BOUND__ = false;

        // Remove any existing records.js script to avoid duplicates
        const existingScript = document.querySelector('script[data-records-script]');
        if (existingScript) existingScript.remove();

        // Inject records.js dynamically
        const script = document.createElement("script");
        script.src = "/static/js/records.js";
        script.setAttribute("data-records-script", "true");
